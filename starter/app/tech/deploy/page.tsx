...
export default function TechDeployPage() {
  ...
  async function handleSubmit(): Promise<void> {
    if (!asset) return;
    if (!isCompleteDeployLocation(location)) {
      setError(
        new ApiError(
          422,
          "incomplete_deploy_location",
          "Deploy needs site, room, rack, and RU.",
        ),
      );
      return;
    }
    ...
    setError(null);
    try {
      const result = await clientScans.deploy({
    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="text-sm text-gray-600 mt-1">
          Rack it and put it into service. We&rsquo;ll write the rack to
          facilities and capitalize it in finance.
        </p>
      </header>
